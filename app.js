 ...
 * Fully static version — all data stored in localStorage.
...
const DEFAULT_CORE_HABITS = [
    { id: 'wakeup',  label: 'Wake up before 7am',      done: false },
    { id: 'workout', label: 'Workout / Exercise',       done: false },
    { id: 'study',   label: 'Study / Deep Work (2h+)',  done: false },
    { id: 'nosugar', label: 'No Junk Food',             done: false },
    { id: 'read',    label: 'Read for 20 mins',         done: false },
...
function storageKey(dateKey) {
    return `tracker_${dateKey}`;
}

function getDefaultDayData() {
    return {
        water: 0,
        coreHabits: DEFAULT_CORE_HABITS.map(h => ({ ...h })),
        customTasks: [],
        learning: '',
    };
}
...
function loadOrCreateToday() {
    const key = getTodayKey();
    let data = loadDay(key);
    if (!data) {
        data = getDefaultDayData();
        saveDay(key, data);
    }
    return data;
}

// ============ API LAYER (localStorage drop-in) ============
const API = {
    async getToday() {
        return loadOrCreateToday();
    },
    ...
    async getHistory() {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (k && k.startsWith('tracker_')) {
                try {
                    const data = JSON.parse(localStorage.getItem(k));
                    data._dateKey = k.replace('tracker_', '');
                    days.push(data);
                } catch { /* skip corrupt entries */ }
            }
        }
        days.sort((a, b) => (b._dateKey || '').localeCompare(a._dateKey || ''));
        return days;
    },
    ...
    async updateWater(water) {
        const key = getTodayKey();
        const data = loadOrCreateToday();
        data.water = Math.max(0, Math.min(5, water));
        saveDay(key, data);
        return data;
    },
    ...
    async toggleHabit(id) {
        const key = getTodayKey();
        const data = loadOrCreateToday();
        for (const habit of data.coreHabits) {
            if (habit.id === id) {
    ...
    async addTask(label) {
        const key = getTodayKey();
        const data = loadOrCreateToday();
        const taskId = `custom_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
        data.customTasks.push({ id: taskId, label, done: false });
        saveDay(key, data);
        return data;
    },
